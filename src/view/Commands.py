...
class Commands:
    ...
    def print_title_block(self, title, end_space = True, end_character = "*"):
        ...
        if title_length <= self._line_width - 4:
            ...
            print(TextFormatter.center_pad_text(title.upper(), self._line_width, end_character))
        else:
            split_titles = TextFormatter.split_oversized_text(title, self._line_width, end_character)
            print(end_character * self._line_width) 
  
            for short_title in split_titles:
                print(TextFormatter.center_pad_text(short_title.upper(), self._line_width, end_character))
